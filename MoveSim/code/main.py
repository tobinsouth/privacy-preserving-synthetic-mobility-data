...
def main(opt):
    ...
    individualEval = IndividualEval(data=opt.data)

    gps = get_gps('../data/%s/gps'%opt.data)
    TOTAL_LOCS = len(gps[0])
    del gps
    ...
    # if opt.data != 'cuebiq':
    gen_matrix(opt.data) # Commented for speed & debugging
    ...
    if opt.pretrain:
        ...
        os.makedirs(DATA_PATH+'/%s/pretrain' % opt.data, exist_ok=True)
    ...
    for epoch in range(EPOCHS):
        ...
        for it in range(1):
            ...
            inputs = torch.cat([zeros, samples.data], dim=1)[
                              :, :-1].contiguous()
            tim = torch.LongTensor([i%24 for i in range(SEQLEN)]).to(device)
